...
Предупреждения (если будут):''')
                ...
                for rs_id in rs_ids:
                        ...
                        #пользователя не обнаружился в
                        #базах, значит он - некорректный.
                        #В матрицу он допущен не будет.
                        else:
                                print(f'{rs_id} - невалидный идентификатор, в матрицу не пойдёт')
                                continue
                        
                        #Мультиаллельные SNP пока не поддерживаются
                        #бэкэндом, следовательно, будут пропущены.
                        if row[4].find(',') != -1:
                                print(f'{rs_id} - мультиаллельный SNP, в матрицу не пойдёт')
                                continue
