...
Допустим, вы создали аннотированную тепловую карту, т.е. с надписями. Самая частая неприятность - когда надписи наползают друг на друга. Всплывающие тексты в статью и презентацию не встроить — в этом случае максимальную информативность диаграммы можно обеспечить лишь обычными надписями. Если диаграмма не очень большая (в пределах 50х50), то проблема наползания решается уменьшением шрифта. В Plotly размер шрифта по умолчанию - 12, поэтому в качестве примера уменьшенного шрифта здесь и далее привожу 10. Смело пробуйте и другие значения. Если что, объект диаграммы можно будет пересоздать (см. ниже), тем самым сбросив ваши изменения.
