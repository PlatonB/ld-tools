...
#### Windows.
Из-за ошибок установки `plyvel` и `pysam` нативной поддержки Windows нет. Но через WSL вроде должно работать. Как будут силы, напишу соответствующую инструкцию.
...
Объект квадратной тепловой карты, в целом, готов. Но четвёртая версия Plotly порождает визуальную шероховатость: серый бэкграунд будет чуточку вылезать за бока диаграммы. Выход из положения — сделать бэкграунд белым.
```
